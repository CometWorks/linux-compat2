...
/// <c>Temp/CrashReports</c> and the game log records the exception, so nothing is lost for
/// local debugging. Minidumps are redirected in <see cref="Platform.LinuxProcessEnvironment"/>,
/// which points Valve's breakpad handler at the game's <c>Temp/MiniDumps</c> folder instead
/// of <c>/tmp/dumps</c>, from where Steam uploads them.
/// </summary>
internal static class CrashReportingPatch
{
    public static void Install()
    {
        ...
        MethodInfo initialize = AccessTools.DeclaredMethod(typeof(CrashHandler), "Initialize")
            ?? throw new MissingMethodException(typeof(CrashHandler).FullName, "Initialize");
        harmony.Patch(initialize,
            prefix: new HarmonyMethod(AccessTools.DeclaredMethod(typeof(CrashReportingPatch), nameof(InitializePrefix))!));
        ...
        {
            MethodInfo target = AccessTools.DeclaredMethod(typeof(CrashHandler), name)
                ?? throw new MissingMethodException(typeof(CrashHandler).FullName, name);
            harmony.Patch(target,
                prefix: new HarmonyMethod(AccessTools.DeclaredMethod(typeof(CrashReportingPatch), nameof(SkipSendPrefix))!));
        }
    }
