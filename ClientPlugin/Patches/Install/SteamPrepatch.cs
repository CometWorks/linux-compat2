...
public static class SteamPrepatch
{
    ...
    private static TypeDefinition FindType(ModuleDefinition module, string fullName) =>
        module.GetType(fullName) ?? throw new InvalidOperationException(
    ...
    private static MethodDefinition FindMethod(TypeDefinition type, string name) =>
        type.Methods.SingleOrDefault(method => method.Name == name) ?? throw new InvalidOperationException(
    ...
    private static int FindSingleCall(MethodDefinition method, string calleeName)
    {
        ...
        {
            if (body[i].OpCode.Code is not (Code.Call or Code.Callvirt)
                || body[i].Operand is not MethodReference callee || callee.Name != calleeName)
                continue;
            if (index != -1)
    ...
    private static void PatchRefreshSubscribedItemSet(ModuleDefinition module)
    {
        MethodDefinition method = FindMethod(
            FindType(module, "Keen.VRage.Steam.UGC.SteamUGCServiceComponent"), "RefreshSubscribedItemSet");
        ...
        foreach (string callee in new[] { "GetNumSubscribedItems", "GetSubscribedItems" })
        {
            ...
            var extended = new MethodReference(original.Name, original.ReturnType, original.DeclaringType)
            {
    ...
    {
        TypeDefinition component = FindType(module, "Keen.VRage.Steam.EngineComponents.SteamGameServiceComponent");
        ...
        FieldDefinition steamUserId = component.Fields.SingleOrDefault(field => field.Name == "_steamUserId")
        ...
        if (body[restart - 2].OpCode.Code != Code.Ldarg_0
            || body[restart - 1].OpCode.Code != Code.Ldfld
            || body[restart + 1].OpCode.Code is not (Code.Brfalse_S or Code.Brfalse))
            throw new InvalidOperationException("[LinuxCompat] RestartAppIfNecessary anchor mismatch.");
        ...
        if (statsCall.Next?.OpCode.Code != Code.Pop)
            throw new InvalidOperationException("[LinuxCompat] RequestCurrentStats anchor mismatch.");
        ...
        var requestUserStats = new MethodReference(
            "RequestUserStats", steamApiCall, currentStats.DeclaringType);
    ...
    private static TypeReference FindSteamApiCallType(ModuleDefinition module, IMetadataScope steamworksScope)
    {
        TypeReference? existing = module.GetTypeReferences()
            .FirstOrDefault(reference => reference.FullName == "Steamworks.SteamAPICall_t");
        if (existing != null)
            return existing;

        return new TypeReference("Steamworks", "SteamAPICall_t", module, steamworksScope, valueType: true);
    }
    ...
    private static void RetargetReferences(MethodDefinition method, Instruction from, Instruction to)
    {
