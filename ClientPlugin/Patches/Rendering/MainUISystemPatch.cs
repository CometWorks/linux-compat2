...
public static class MainUISystemPatch
{
    ...
    private static readonly Dictionary<RenderDrawCommandBuffer, Vector2> SubmittedBatchResolutions =
        new();
    ...
    public static void BatchSubmitted(RenderDrawCommandBuffer drawBatch, int sortLayer)
    {
        ...
        {
            if (
                SubmittedBatchResolutions.Remove(drawBatch, out Vector2 resolution)
                && sortLayer == 100
                && drawBatch.RenderTarget == default
            )
