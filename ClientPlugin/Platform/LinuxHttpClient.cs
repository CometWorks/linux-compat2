...
internal sealed class LinuxHttpClient : IPlatformHttp
{
    ...
    public Keen.VRage.Library.Threading.Task<(
        HttpStatusCode,
        string?
    )> SendFormattedCrashReportAsync(
        string url,
        string jsonBody,
    ...
    )
    {
        ...
        await using Stream source = await response
            .Content.ReadAsStreamAsync()
            .ConfigureAwait(false);
        await using Stream destination = file.Open(
            FileMode.Create,
            FileAccess.Write,
            FileShare.Read
        );
        ...
        progressCallback?.Invoke(
            destination.Length,
            response.Content.Headers.ContentLength ?? destination.Length
        );
        return response.StatusCode;
    }

    private static HttpMethod ToHttpMethod(IPlatformHttp.Method method) =>
        method switch
        {
