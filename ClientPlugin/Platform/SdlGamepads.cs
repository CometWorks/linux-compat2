...
internal static class SdlGamepads
{
        ...
        switch (e.Type)
        {
            ...
            case GamepadAxisMotion:
                if (Devices.TryGetValue(id, out OpenDevice? gamepad) && gamepad.IsGamepad)
                    Pending.Enqueue(
                        SdlGamepadEvent.Axis(id, e.GamepadAxis.Axis, e.GamepadAxis.Value)
                    );
                break;
            ...
            case GamepadButtonUp:
                if (Devices.TryGetValue(id, out gamepad) && gamepad.IsGamepad)
                    Pending.Enqueue(
                        SdlGamepadEvent.Button(
                            id,
                            MapGamepadButton(e.GamepadButton.Button),
            ...
            case JoystickAxisMotion:
                if (Devices.TryGetValue(id, out OpenDevice? joystick) && !joystick.IsGamepad)
                    Pending.Enqueue(
                        SdlGamepadEvent.Axis(
                            id,
                            MapJoystickAxis(e.GamepadAxis.Axis),
                            e.GamepadAxis.Value
                        )
                    );
                break;
            ...
            case JoystickButtonUp:
                if (Devices.TryGetValue(id, out joystick) && !joystick.IsGamepad)
                    Pending.Enqueue(
                        SdlGamepadEvent.Button(
                            id,
                            MapJoystickButton(e.GamepadButton.Button),
            ...
            case JoystickHatMotion:
                if (
                    Devices.TryGetValue(id, out joystick)
                    && !joystick.IsGamepad
                    && e.JoystickHat.Hat == 0
                )
    ...
    private static void Open(uint id)
    {
        ...
        if (handle == 0)
        {
            Console.Error.WriteLine(
                $"[LinuxCompat] SDL could not open input device {id}: {GetError()}"
            );
            return;
        }

        string name =
            GetString(isGamepad ? SDL_GetGamepadNameForID(id) : SDL_GetJoystickNameForID(id))
            ?? $"SDL input {id}";
        string? serial = GetString(
            isGamepad ? SDL_GetGamepadSerial(handle) : SDL_GetJoystickSerial(handle)
        );
        string? path = GetString(
            isGamepad ? SDL_GetGamepadPathForID(id) : SDL_GetJoystickPathForID(id)
        );
        ...
        if (
            serial != null
            && path != null
            && Devices.Values.Any(device => device.DeviceId == deviceId)
        )
            deviceId += $":{path}";
        ...
        Console.WriteLine(
            $"[LinuxCompat] SDL input connected: {name} ({(isGamepad ? "gamepad" : "joystick")})"
        );
    }
    ...
    private static void QueueSnapshot(uint id, OpenDevice device)
    {
        if (device.IsGamepad)
        {
            for (int axis = 0; axis < 6; axis++)
                Pending.Enqueue(
                    SdlGamepadEvent.Axis(id, axis, SDL_GetGamepadAxis(device.Handle, axis))
                );
            for (int button = 0; button < 15; button++)
                Pending.Enqueue(
                    SdlGamepadEvent.Button(
                        id,
                        MapGamepadButton(button),
    ...
    private static int MapGamepadButton(int button) =>
        button switch
        {
            0 => 0,
            1 => 1,
    ...
    private static int MapJoystickButton(int button) =>
        button switch
        {
            0 => 1,
            1 => 0,
    ...
    private static int MapJoystickAxis(int axis) =>
        axis switch
        {
            0 => 0,
            1 => 1,
    ...
    private static string? GetString(nint value) =>
        value == 0 ? null : Marshal.PtrToStringUTF8(value);
    ...
    [DllImport(Sdl)]
    private static extern nint SDL_GetJoysticks(out int count);
    ...
    [DllImport(Sdl)]
    private static extern nint SDL_OpenGamepad(uint instanceId);

    [DllImport(Sdl)]
    private static extern void SDL_CloseGamepad(nint gamepad);

    [DllImport(Sdl)]
    private static extern nint SDL_OpenJoystick(uint instanceId);

    [DllImport(Sdl)]
    private static extern void SDL_CloseJoystick(nint joystick);

    [DllImport(Sdl)]
    private static extern nint SDL_GetGamepadNameForID(uint instanceId);

    [DllImport(Sdl)]
    private static extern nint SDL_GetJoystickNameForID(uint instanceId);

    [DllImport(Sdl)]
    private static extern nint SDL_GetGamepadPathForID(uint instanceId);

    [DllImport(Sdl)]
    private static extern nint SDL_GetJoystickPathForID(uint instanceId);

    [DllImport(Sdl)]
    private static extern SdlGuid SDL_GetGamepadGUIDForID(uint instanceId);

    [DllImport(Sdl)]
    private static extern SdlGuid SDL_GetJoystickGUIDForID(uint instanceId);

    [DllImport(Sdl)]
    private static extern nint SDL_GetGamepadSerial(nint gamepad);

    [DllImport(Sdl)]
    private static extern nint SDL_GetJoystickSerial(nint joystick);
    ...
    [DllImport(Sdl)]
    private static extern int SDL_GetNumJoystickAxes(nint joystick);

    [DllImport(Sdl)]
    private static extern int SDL_GetNumJoystickButtons(nint joystick);

    [DllImport(Sdl)]
    private static extern int SDL_GetNumJoystickHats(nint joystick);

    [DllImport(Sdl)]
    private static extern short SDL_GetJoystickAxis(nint joystick, int axis);
    ...
    [DllImport(Sdl)]
    private static extern byte SDL_GetJoystickHat(nint joystick, int hat);
...
)
{
    ...
    internal static SdlGamepadEvent Disconnected(uint id) =>
        new(SdlGamepadEventKind.Disconnected, id);
